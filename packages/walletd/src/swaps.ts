...
/**
 * Seller: pre-sign a swap offer on one of OUR 1-sat inscribed carriers.
 * Proceeds always pay our own wallet — the page names the outpoint and
 * the price, never the payee. Off-chain (no broadcast, nothing tracked).
 */
...
export async function completeSwap(opts: {
  ...
  if (o.version !== SWAP_VERSION || o.lockTime !== SWAP_LOCKTIME) fail("BAD_OFFER", "offer version/locktime mismatch");
  ...
  await track(opts.db, res.txid, `swap buy ${price} sats from ${parts.txid.slice(0, 8)}`, hex);
